...
# --- NODE 1: THE INDEXER ---
# Downloads video from YT, uploads to blob storage, gets transcript, ocr text using video indexer and outputs as json
def index_video_node(state: VideoAuditState) -> Dict[str, Any]:
    ...
    try:
        ...
        # 4. WAIT
        raw_insights = vi_service.wait_for_processing(azure_video_id) #pause from running nxt step, while azure waits for video processing
...
# Using RAG to perform audit from transcript and KB
def audit_content_node(state: VideoAuditState) -> Dict[str, Any]:
    ...
    logger.info("---after AzureSearch initlization--------------")

    # RAG Retrieval
    ocr_text = state.get("ocr_text", [])
    query_text = f"{transcript} {' '.join(ocr_text)}"
    docs = vector_store.similarity_search(query_text, k=3) # gets relevant pages from kb based on query_text
    ...
    try:
        ...
        if "```" in content:
            ...
            content = re.search(r"```(?:json)?(.*?)```", content, re.DOTALL).group(1) # removes ``` and gives only json
